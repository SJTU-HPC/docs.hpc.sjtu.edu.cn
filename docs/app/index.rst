...
π集群 有许多预建的软件模块，并且数量还在不断增长。欢迎您告诉我们您研究领域中流行的软件。由于收费很少甚至为零，因此开源软件的安装优先级更高。
...
+------------------------+-----------------+-----------------+
| :ref:`deepvariant`     | 10.0.130        | |cpu| |gpu|     |
+------------------------+-----------------+-----------------+
| :ref:`Keras`           |                 |                 |
+------------------------+-----------------+-----------------+
...
ENVIRONMENT MODULES可以帮助您在π集群上使用预构建的软件包。每个ENVIRONMENT
MODULES都是可以实时应用和不应用的一组环境设置。用户可以编写自己的模块。

本文档将向您介绍ENVIRONMENT MODULES的基本用法以及π集群上可用的软件模块。
