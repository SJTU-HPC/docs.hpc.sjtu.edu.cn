...
!!! danger "sshproxy not working with NoMachine on Windows"
    There is currently a known issue with using sshproxy
    to generate keys on Windows for NoMachine. We are aware
    of this issue and the sshproxy developers are working
    on a fix. We will update this page when the issue has been
    resolved. Until then Windows users will need to follow
    the directions to configure NoMachine **WITHOUT sshproxy**.
