...
（在自己电脑终端上）$ ssh-keygen -t rsa                                # 生成密钥对
（在 HPC 集群上）  $ rm  ~/.ssh/authorized_keys                       # 清除服务器上的原有的 authorized_keys
（在自己电脑终端上）$ ssh-copy-id YOUR_USERNAME@TARGET_IP              # 将新的公钥发给服务器，存在服务器的 authorized_keys 文件里
```
