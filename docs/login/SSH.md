...
（在自己电脑终端上）$ rm  ~/.ssh/id*                                # 清除本地 .ssh 中的密钥对信息
（在自己电脑终端上）$ ssh-keygen -t rsa                           # 在本地生成密钥对
（在自己电脑终端上）$ ssh-copy-id YOUR_USERNAME@TARGET_IP         # 在本地将新的公钥发给服务器，存在服务器的 authorized_keys 文件里
```
