...
登录节点资源有限，不推荐在登录节点直接进行大批量的数据传输。超算平台提供了专门用于数据传输的节点，登录该节点后可以通过rsync，scp等方式将个人目录下的数据下载到本地，或者反向上传本地数据到个人目录。详情请参考具体请参考\ `数据传输章节 <https://docs.hpc.sjtu.edu.cn/transport/index.html>`__\ 。
...
如果您需要传输小批量数据，直接通过登陆节点传输即可。中等规模的数据传输请登录专用传输节点进行操作，具体请参考\ `数据传输章节 <https://docs.hpc.sjtu.edu.cn/transport/index.html>`__\ 。对于大批量数据，请发送邮件至 `hpc邮箱 <mailto:hpc@sjtu.edu.cn>`_ 预约硬盘传输，并将硬盘邮寄或携带至我们的办公室，具体联系方式我们会通过邮件回复您。
