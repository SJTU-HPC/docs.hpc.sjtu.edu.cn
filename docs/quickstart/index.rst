...
您只需要一个 π 集群帐号，然后在任何一个常见的浏览器上登录 \ `可视化平台 HPC Studio <../studio/>`__\ ，即可自由使用 π 集群。
...
π 集群服务于交大师生。帐号申请者需为交大在职教职工（包含博士后）。每个帐号可免费申请四个子帐号。
...
开通子帐号之前，还需子帐号使用者完成“新手上路”考核（B 站互动视频），共 10 小题（预计完成时间 5 分钟）。正确回答所有问题后截图给我们，稍后我们会在两个工作日内开通帐号。
