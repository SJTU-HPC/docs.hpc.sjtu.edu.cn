...
数据传输节点（data.hpc.sjtu.edu.cn）还多挂载了一个 /union。
...
在 `data （data.hpc.sjtu.edu.cn）` 节点的目录 `/union` 下挂载了冷存储，共 29P 容量，用来存储用户的不常用数据。

冷存储系统特性
----------------

作为 Lustre/GPFS 文件系统的补充，提供大容量、高可用的存储，只能在传输节点访问，主要给用户存放不常用的数据。冷存储系统具有快照功能，快照可以恢复用户误删除的数据。用户通过 ``cd $UNION`` 可以跳转到自己的目录
...
用户可以在以下路径找到 /union 提供的个人存储空间: ``/union/home/acct-xxxx/yyyy``

其中 acct-xxxx 代表计费帐号（课题组帐号），yyyy 代表个人帐号。

为了快捷访问，我们已经为用户设置好环境变量，``cd $UNION`` 即可进入。
...
.. code::
    ...
    # rsync -avh -P --append-verify $DATA $UNION
