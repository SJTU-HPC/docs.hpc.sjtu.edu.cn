...
在转移 ``/lustre`` 下的数据时，需要选择 data 传输节点，在转移 ``/dssg`` 下的数据时，需要选择 sydata 传输节点。
如果要使用已经迁移到冷存储的数据，用户需要手动将数据从冷存储传输到 Lustre/GPFS。
...
假设用户 expuser01 需要将 Lustre 个人目录下的数据 ``$HOME/data/`` 搬运到冷存储下的个人目录 ``$ARCHIVE/data/``\ ，需要执行的命令为：
...
如果需要使用冷存储中的数据，用户需要先手动将数据传输到 Lustre/GPFS。
和迁移数据到冷存储类似，需要完成数据传输和数据校验，具体命令可以参考上面的“数据传输”和“数据校验”部分。