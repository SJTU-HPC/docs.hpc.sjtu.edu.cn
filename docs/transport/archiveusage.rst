...
科学数据平台（冷存储系统）挂载在传输节点（ ``data`` 节点、 ``sydata`` 节点）以及计算节点，挂载路径为 ``/union`` 。
...
Pi集群、思源一号集群的传输节点 ``data``、``sydata`` 都挂载了冷存储，可以访问并修改冷存储数据。
迁移数据请使用传输节点。在转移 Pi 超算 ``/lustre`` 下的数据时，需要选择 data 传输节点，在转移思源一号 ``/dssg`` 下的数据时，需要选择 sydata 传输节点。

**登录节点、计算节点**

为了方便用户使用冷存储中的数据，Pi、思源一号集群的登录节点、计算节点也挂载了冷存储。挂载方式为只读，不能写入数据。

传输数据到冷存储
----------------------

迁移数据请使用传输节点。``rsync`` 支持增量传输、断点续传、文件校验等功能，建议用 ``rsync`` 命令拷贝数据。
...
假设用户 expuser01 需要将 Pi 超算个人目录下的数据 ``$HOME/data/`` 搬运到冷存储下的个人目录 ``$UNION/data/``\ ，需要执行的命令为：

.. code:: bash

   # -a 表示保存所有元数据，-v 打印更多信息，-r 表示包含子目录，-P 表示显示进展以及保存未传输完的文件，其余可用参数见 rsync 文档
   rsync -avrP $HOME/data/ $UNION/data/

   # 如果要搬运的目录中包含软链接文件，需要用 --copy-unsafe-links 参数，将不安全的软链接（原文件不在备份范围内）转换成实体文件保存
   rsync -avrP --copy-unsafe-links $HOME/data/ $UNION/data/

   # 如果要搬运的目录中包含硬链接文件，可以用 -H 参数，保留文件之间的硬链接信息，减少不必要的文件传输
   rsync -avrPH $HOME/data/ $UNION/data/

**使用 rsync 从冷存储向 Pi 超算或思源一号传输数据**

假设用户 expuser01 需要将冷存储下的个人目录 ``$UNION/data/`` 搬运到 Pi 超算个人目录下的数据 ``$HOME/data/``\ ，需要执行的命令为：

.. code:: bash

   # -a 表示保存所有元数据，-v 打印更多信息，-r 表示包含子目录，-P 表示显示进展以及保存未传输完的文件，其余可用参数见 rsync 文档
   rsync -avrP $UNION/data/ $HOME/data/

   # 如果要搬运的目录中包含软链接文件，需要用 --copy-unsafe-links 参数，将不安全的软链接（原文件不在备份范围内）转换成实体文件保存
   rsync -avrP --copy-unsafe-links $UNION/data/ $HOME/data/

   # 如果要搬运的目录中包含硬链接文件，可以用 -H 参数，保留文件之间的硬链接信息，减少不必要的文件传输
   rsync -avrPH $UNION/data/ $HOME/data/
...
数据传输可能受网络波动影响，建议在数据传输完成之后，通过数据校验确认数据完整。
...
假设用户 expuser01 需要为目录 ``$HOME/data/`` 和 ``$UNION/data/`` 下的子目录的每个文件生成 md5 值，需要执行以下命令：
...
.. code:: bash
   ...
   md5deep -rl $UNION/data/ > after.md5deep
...
在完成数据传输、数据校验之后，可以清理Pi存储、思源存储上的原文件。
...
.. code:: bash
   ...
   # 假设原文件存储位置在 /union/home/acct-exp/expuser01/data/
   rm -rf /union/home/acct-exp/expuser01/data/